...
- Markdown 导入会创建独立 Case；顶部 Case 选择器可查看和切换所有内置及导入记录
- 每次编辑会自动保存到 `annotation-edits.json`，服务重启后恢复每个 Case 的最新时间轴、音频和轨道状态
- 任意当前 Case 都可导出单文件 HTML；导出器只打包该 Case 的最终 TTS、上传音频、背景音和任务音
